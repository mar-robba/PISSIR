...
Il diagramma raccoglie i servizi che il sistema offre agli utenti (RF01) e la ripartizione dei
permessi fra i due ruoli (RF04.2). Ogni caso d'uso porta accanto il requisito che lo enuncia, cosi'
il disegno e il capitolo si leggono uno accanto all'altro.

Quattro cose vanno dette prima di leggerlo:

- *Utente* non e' un terzo ruolo: e' l'attore astratto che raccoglie cio' che tecnico e
  amministratore fanno entrambi, cioe' la consultazione e l'accesso. I due ruoli reali lo
  specializzano, quindi ereditano tutte le sue associazioni e il disegno non deve ripeterle due
  volte.
- I ruoli dei *comandi operativi* sono quelli della consegna: la squadra sulla stazione guasta la
  manda il tecnico, la corsa la sopprime l'amministratore. La chiusura dell'allarme sta sul tecnico
  perche' e' il seguito dell'invio degli operatori, che e' compito suo.
- Il *tecnico* e' chi usa l'applicazione, non la squadra che va sul posto: e' lui a mandare gli
  operatori, non a essere mandato. Nel diagramma vecchio /invia Tecnico/ partiva dall'attore tecnico,
  ed era il verso sbagliato.
- La *marcatura* dei casi d'uso, spiegata nella legenda, dice due cose insieme: quanto di quel caso
  d'uso e' realizzato e se la consegna lo chiede. Nessun caso d'uso e' del tutto mancante; tre sono
  realizzati a meta' e sono segnati =«parziale»=.

Restano fuori dal disegno tre cose, e nessuna per dimenticanza: RF01.1.4, l'aggiornamento continuo
delle schermate, perche' e' una proprieta' di tutte le visualizzazioni e non un caso d'uso a se'; i
nodi di campo, perche' non sono utenti e non si identificano con credenziali, la loro verifica e'
RF02.4; l'inclusione di /Accedi al sistema/ da parte di ogni altro caso d'uso, che vale per tutti e
disegnata su tutti renderebbe il diagramma illeggibile.
...
La consegna descrive i servizi della centrale in due elenchi, uno per il tecnico e uno per
l'amministratore. Questa tabella li rilegge uno per uno e dice dove sono finiti nella specifica,
cosi' si vede subito che cosa e' coperto e con quale riserva.

#+attr_latex: :align |p{0.09\textwidth}|p{0.40\textwidth}|p{0.18\textwidth}|p{0.23\textwidth}|
| ruolo | cosa chiede la consegna                                              | requisito         | stato                                    |
|-------+----------------------------------------------------------------------+-------------------+------------------------------------------|
| tecnico | visualizza la situazione corrente del traffico e le tratte          | RF01.1.1, RF01.1.2 | completo                                |
| tecnico | quali tratte segue un certo treno                                   | RF01.1.2, RF01.3.4 | completo, in lettura                    |
| tecnico | quali treni sono attesi in ingresso e in uscita in una stazione     | RF01.1.3          | completo                                 |
| tecnico | manda gli operatori a riparare una stazione guasta                  | RF01.4.1          | parziale: MANUTENZIONE non dura          |
| amministratore | visualizza la situazione corrente del traffico e le tratte   | RF01.1, RF01.2    | completo, come il tecnico                |
| amministratore | aggiunge ed elimina tratte                                   | RF01.3.3          | completo                                 |
| amministratore | modifica le stazioni attraversate dai treni                  | RF01.3.4, RF02.5  | parziale: il convoglio in marcia riparte dal capolinea |
| amministratore | (eventualmente) sopprime treni fermi in stazione             | RF01.4.3          | parziale: la precondizione non e' verificata |
| entrambi | il server e' accessibile da due tipologie di utenti con permessi diversi | RF04.1, RF04.2 | parziale: sui tre comandi operativi il ruolo non viene distinto |

Quello che c'e' in piu' rispetto alla consegna e' segnato =«aggiunta»= nel diagramma, e resta nella
specifica perche' e' realizzato e funzionante: gli indicatori di sintesi (RF01.1.5), la consultazione
dello storico dei transiti (RF01.5 — la consegna chiede di *memorizzarlo*, che e' RF02.7, non di
mostrarlo), la gestione delle stazioni (RF01.3.1) e quella dei convogli (RF01.3.2). Le ultime due
non sono un capriccio: la consegna da' per scontata una rete gia' popolata, e senza un posto in cui
crearla non ci sarebbero ne' le tratte da aggiungere ne' i treni a cui cambiare le stazioni
attraversate.

Un punto va detto esplicitamente perche' e' l'unico in cui il sistema realizzato e' piu' *permissivo*
della consegna: i tre comandi operativi sono concessi a tutti e due i ruoli, mentre la consegna ne
assegna uno al tecnico e uno all'amministratore. Il diagramma disegna la ripartizione della consegna,
il controllo dei permessi non la fa rispettare, ed e' per questo che RF04.2 e' parziale.
...
** COMMENT Grafo delle dipendenze fra i requisiti
...
- e' la vista d'insieme, a livello di moduli e non del loro interno: le cinque componenti eseguibili (web app, Centrale, Stazione, Treno, broker) piu' i due servizi di supporto che girano in Docker, Keycloak e PostgreSQL
- le interfacce non hanno nomi di comodo: dentro ci sono i percorsi REST, il percorso del WebSocket e i topic MQTT veri, quelli che stanno in =application.properties= e nelle classi di ingresso
- linea continua = interfaccia *offerta* dalla componente, freccia tratteggiata = interfaccia *richiesta*
...
Le cose da leggere nel diagramma sono quattro.

*La web app parla su tre canali diversi e non su uno solo.* Le chiamate =/api= servono per
leggere l'anagrafica e per mandare i comandi, il WebSocket =/ws/realtime= porta gli
aggiornamenti che partono dalla Centrale senza che nessuno li chieda, e il terzo canale non
tocca affatto la Centrale: il login va diretto a Keycloak, ed e' il motivo per cui
=IAutenticazione= e =IChiaviPubbliche= sono due interfacce separate. La prima la usa il
browser per farsi dare il token, la seconda la usa la Centrale per verificarne la firma
senza richiamare Keycloak a ogni richiesta.

*Dal campo verso la Centrale si passa dal broker, tranne un caso.* Telemetria, transiti,
battiti, validazioni degli ID e allarmi viaggiano tutti su =ITopicTree=: la Centrale e i nodi
non si conoscono e non si chiamano fra loro. L'unica eccezione e' il treno che al boot
scarica il proprio itinerario in HTTP (=IApiDiCampo=), perche' e' un dato di configurazione
che serve una volta sola e che non ha senso far girare come messaggio.

*I sensori entrano solo dalle API REST del nodo.* =ISensoriStazione= e =ISensoriTreno= sono le
uniche porte di ingresso dei sensori: un sensore non pubblica su MQTT e non conosce la
Centrale, parla con il nodo che ha vicino e basta. Sono disegnati con lo stereotipo
/esterno/ perche' stanno fuori dal sistema, e nel progetto sono simulati: quelli di bordo
dalla console dei sensori del treno, quelli di stazione da chiamate REST fatte a mano.

*Una sola componente scrive su disco.* Stazione e Treno tengono il loro stato in memoria e
non hanno nessun database: l'unica che parla con PostgreSQL e' la Centrale. E' anche il motivo
per cui la stazione, quando la rete cade, si tiene i messaggi in una cache locale e li rimanda
al rientro invece di scriverli da qualche parte.
** Stack tecnologico
#+begin_comment
