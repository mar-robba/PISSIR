...
#+begin_src  plantuml file: casi_uso_00_attori_e_casi_uso.png
@startuml UC0 - Attori e casi d'uso del sistema
' =========================================================================
'  MODELLAZIONE PER CASI D'USO
'
'  Il diagramma copre RF01 (i servizi che il sistema offre agli utenti) e la
'  ripartizione dei permessi di RF04.2, e segue la consegna: il tecnico
'  visualizza e manda gli operatori sulla stazione guasta, l'amministratore
'  visualizza, tiene l'anagrafica della rete e sopprime le corse.
'
'  Come e' costruito:
'    - Utente e' l'attore astratto che raccoglie cio' che i due ruoli reali
'      fanno entrambi (la consultazione e l'accesso); tecnico e
'      amministratore lo specializzano e ne ereditano le associazioni;
'    - ogni caso d'uso porta fra parentesi il requisito che lo enuncia;
'    - la marcatura dice quanto ne e' realizzato e se la consegna lo chiede
'      (vedi la legenda in fondo al diagramma).
'
'  I nodi di campo (convogli e stazioni) non compaiono: non sono utenti e
'  non si identificano con credenziali, il loro riconoscimento e' RF02.4.
' =========================================================================
...
skinparam usecase<<parziale>> {
    BackgroundColor #FEF7E0
    BorderColor #F29900
}
skinparam usecase<<aggiunta>> {
...
actor "Servizio di identita'" as IDP <<sistema>>

TECNICO --|> UTENTE

rectangle "Monitoraggio e gestione del traffico ferroviario" {

  rectangle "Consultazione (RF01.1, RF01.2, RF01.5)" as G1 {
    usecase "Visualizza la mappa\ndel traffico\n(RF01.1.1)" as UC01
    usecase "Consulta l'elenco\ndei convogli\n(RF01.1.2)" as UC02
    usecase "Consulta l'elenco\ndelle stazioni\n(RF01.1.3)" as UC03
    usecase "Consulta le tratte\ne gli itinerari\n(RF01.3, sola lettura)" as UC06
    usecase "Consulta gli allarmi\naperti\n(RF01.2)" as UC05
    usecase "Consulta gli indicatori\ndi sintesi\n(RF01.1.5)" as UC04 <<aggiunta>>
    usecase "Consulta lo storico\ndei transiti\n(RF01.5)" as UC07 <<aggiunta>>
  }

  rectangle "Comandi operativi (RF01.4)" as G2 {
    usecase "Invia la squadra\ndi manutenzione\n(RF01.4.1)" as UC08 <<parziale>>
    usecase "Dichiara risolto\nun allarme\n(RF01.4.2)" as UC09 <<parziale>>
    usecase "Sopprimi una corsa\n(RF01.4.3)" as UC10 <<parziale>>
  }

  rectangle "Accesso (RF01.6)" as G4 {
    usecase "Accedi al sistema" as UC15
    usecase "Esci dal sistema" as UC16
  }

  rectangle "Anagrafica della rete (RF01.3)" as G3 {
    usecase "Gestisci le tratte\nelementari\n(RF01.3.3)" as UC13
    usecase "Gestisci gli itinerari e\nl'assegnazione dei convogli\n(RF01.3.4)" as UC14
    usecase "Gestisci le stazioni\n(RF01.3.1)" as UC11 <<aggiunta>>
    usecase "Gestisci i convogli\n(RF01.3.2)" as UC12 <<aggiunta>>
  }
}

' ---- cio' che i due ruoli fanno entrambi ----
UTENTE -- UC01
UTENTE -- UC02
UTENTE -- UC03
UTENTE -- UC04
...
' ---- compito del tecnico: la stazione guasta e il suo ripristino ----
TECNICO -- UC08
TECNICO -- UC09
...
' ---- le credenziali si digitano fuori dall'applicazione (RF04.1) ----
UC15 -- IDP
UC16 -- IDP

' ---- vincoli di sola impaginazione: distribuiscono i gruppi su tre file ----
UC01 -[hidden]down- UC08
UC02 -[hidden]down- UC09
UC03 -[hidden]down- UC10
UC06 -[hidden]down- UC15
UC05 -[hidden]down- UC16
UC08 -[hidden]down- UC13
UC09 -[hidden]down- UC14
UC10 -[hidden]down- UC11
UC15 -[hidden]down- UC12

legend bottom
  <b>Marcatura dei casi d'uso</b>
  nessuna marca : chiesto dalla consegna e realizzato per intero
  <b>«parziale»</b> : chiesto dalla consegna, realizzato solo in parte; il pezzo
  che manca sta nello stato di implementazione del requisito
  <b>«aggiunta»</b> : non chiesto dalla consegna, realizzato lo stesso
endlegend
@enduml
#+end_src

#+RESULTS:
[[file:/tmp/babel-25gpgG/plantuml-5VTOFG.png]]
...
#+begin_src  plantuml file: diagramma_delle_componenti.png
@startuml Diagramma delle componenti
' =========================================================================
'  DIAGRAMMA DELLE COMPONENTI
'
'  Solo i moduli, non il loro interno: le cinque componenti eseguibili del
'  sistema (web app, Centrale, Stazione, Treno, broker) piu' i due servizi
'  di supporto che girano in Docker, Keycloak e PostgreSQL.
'
'  I nomi delle interfacce non sono nomi di comodo: dentro ci sono i
'  percorsi e i topic veri, presi da
'    */src/main/resources/application.properties      (porte, topic, %tls)
'    ServeCentraleOperativa/.../gateway/              (/api, /ws/realtime)
'    Stazioni/.../StationIngestion.java               (/stazione/sensore/*)
'    Treni/.../TrainIngestion.java                    (/treno/sensore/*)
'
'  Convenzione: linea continua = interfaccia OFFERTA dalla componente,
'  freccia tratteggiata = interfaccia RICHIESTA (dipendenza).
'
'  Lo stereotipo <<esterno>> sta solo sui sensori: non sono moduli miei ne'
'  servizi di supporto, sono i dispositivi che stanno fuori dal sistema e
'  che nel progetto vengono simulati (console dei sensori del treno,
'  chiamate REST a mano per la stazione).
' =========================================================================
...
skinparam component<<centrale>> {
    BackgroundColor #FCE8E6
    BorderColor #C5221F
}
skinparam component<<campo>> {
...
title Diagramma delle componenti

' =====================================================
'  COMPONENTI
' =====================================================
component "Client Web App\nReact 18 + TypeScript + Vite" <<webapp>> as WebApp
component "Keycloak 26\nIdentity Provider (Docker)" <<infra>> as Keycloak
component "Centrale Operativa\nQuarkus :8781 (:8444 tls)" <<centrale>> as Centrale
database "PostgreSQL 15\ndb railway (Docker)" as DB
component "Broker MQTT\nEclipse Mosquitto (Docker)" <<infra>> as Broker
component "Stazione\nQuarkus :8081 (N istanze)" <<campo>> as Stazione
component "Treno\nQuarkus :8082 (N istanze)" <<campo>> as Treno
component "Sensori di stazione\npassaggio, guasto, keepalive\n(simulati)" <<esterno>> as SensoriStaz
component "Sensori di bordo\nguasto, passaggio\n(simulati)" <<esterno>> as SensoriTreno

' =====================================================
'  INTERFACCE
' =====================================================
interface "IAutenticazione\nOAuth2/OIDC :8180" as IAuth
interface "IChiaviPubbliche\nJWKS" as IJwks
interface "IApiOperatore\nREST /api\n:8781 (:8444 tls)" as IApi
interface "ITempoReale\nWebSocket /ws/realtime" as IRealtime
interface "IApiDiCampo\nGET /api/treni/{id}/itinerario" as IApiCampo
interface "ITopicTree\nMQTT railway/#\n:1883 (:8883 tls)" as ITopic
interface "IPersistenza\nJDBC :5432" as IPersistenza
interface "ISensoriStazione\nREST /stazione/sensore/*\n:8081" as ISensStaz
interface "ISensoriTreno\nREST /treno/sensore/*\n:8082" as ISensTreno

' =====================================================
'  LATO OPERATORE
' =====================================================
WebApp .down.> IApi : comandi e letture\ncon Bearer token
WebApp .down.> IRealtime : aggiornamenti spinti\ndalla Centrale
WebApp .right.> IAuth : login PKCE, refresh,\nlogout
Keycloak -up- IAuth
Keycloak -up- IJwks
Centrale .right.> IJwks : firma RS256\ne ruoli del token

Centrale -up- IApi
Centrale -up- IRealtime
Centrale -up- IApiCampo

' =====================================================
'  PERSISTENZA
' =====================================================
Centrale .down.> IPersistenza : storicizza transiti, guasti,\nstati e itinerari
DB -up- IPersistenza

' =====================================================
'  CAMPO
' =====================================================
Centrale .down.> ITopic : sottoscrive telemetria, transiti,\nbattiti e validazioni,\npubblica gli allarmi
Broker -up- ITopic
Stazione .up.> ITopic : pubblica transiti e battiti,\nsottoscrive allarmi
Treno .up.> ITopic : pubblica telemetria e passaggi,\nsottoscrive allarmi
Treno .up.> IApiCampo : scarica l'itinerario\nall'avvio (HTTP)

Stazione -down- ISensStaz
Treno -down- ISensTreno
SensoriStaz .up.> ISensStaz
SensoriTreno .up.> ISensTreno

note as Legenda
  <b>Come si legge</b>
  linea continua = interfaccia <b>offerta</b> dalla componente
  freccia tratteggiata = interfaccia <b>richiesta</b>
  sotto il profilo <i>%tls</i> MQTT passa da 1883 a 8883
  e la Centrale da HTTP 8781 a HTTPS 8444
end note
...
#+RESULTS:
[[file:/tmp/babel-SxrZR3/plantuml-LxXXA5.png]]
...
** COMMENT Autenticazione e autorizzazione
...
- requisiti coperti: RF04.1 completo e RF04.2 parziale (le eccezioni dichiarate nella specifica ci sono tutte — le richieste preliminari del browser e le due sole letture di cui hanno bisogno i nodi di campo — ma i tre comandi operativi restano aperti a entrambi i ruoli), e RF01.6 completo lato centrale: il controllo dei permessi e' fatto qui, dalla parte che possiede i dati, non nascondendo i comandi nell'interfaccia
...
- requisiti coperti: RF01.6 e RF04.1 completi, RF04.2 parziale perche' il ruolo non viene distinto sui tre comandi operativi; RF04.3 non e' una macchina ne' una sequenza, e' il profilo tls sui canali
...
activate REST
REST -> PG : cancella i Transiti vivi, poi il Treno\n(lo storico non si tocca: RF02.7)
...
#+RESULTS:
[[file:/tmp/babel-vRTivD/plantuml-FNfkR2.png]]
...
'
'  Le cinque classi di questa vista non toccano piu' il database da se':
'  le query stanno nel RailwayRepository (C5). Qui restano la decisione di
'  che cosa scrivere e il confine della transazione, che per l'ingestione
'  e per il watchdog e' QuarkusTransaction.requiringNew() chiamato dentro
'  il try.
' =========================================================================
...
'  prima erano novantaquattro, sparse in otto classi: RestApiGateway 61,
'  IngestionService 23, TrafficLogicEngine 3, FaultMonitor 3,
...
class RailwayRepository <<ApplicationScoped>> <<persistenza>> {
  ...
  + guastiDerivatiDaConvoglio() : List<Guasto>
  ...
  + salvaEventoStazione(stazioneId, nomeStazione, stato, tipoEvento, descrizione) : void
...
class "entita' JPA\n<size:11>Stazione, Treno, Tratta, Itinerario, ItinerarioTratta, Transito,</size>\n<size:11>Guasto, EventoStazione, Utente e i quattro storici scritti</size>" as ENTITA <<modello>>

RestApiGateway --> RailwayRepository : anagrafica, comandi operativi,\nitinerario dei nodi di campo
AuthController --> RailwayRepository : trovaUtentePerMatricola,\ncon la matricola presa dal token
...
note bottom of ENTITA
  ...
  E' anche il motivo per cui l'unica query che stava dentro un'entita'
  e' sparita: StoricoGuasto.fotografiaDi() cercava da se' il nome della
  stazione sorgente, adesso quel nome glielo passa il repository.
end note
...
- rispetto alla consegna: e' un'aggiunta, la consegna non la chiede.
...
*Rispetto alla consegna*: la consegna chiede all'amministratore di aggiungere ed eliminare le tratte
...
*Chi puo' dare quale comando*: lo dice la consegna, e non e' lo stesso ruolo per tutti e tre. La
squadra sulla stazione guasta la manda il *tecnico*; la corsa la sopprime l'*amministratore*, e la
...
*Limite comune a tutti e tre*: la ripartizione qui sopra e' quella disegnata nei casi d'uso, ma il
...
- enunciato: il tecnico manda la squadra su una stazione guasta.
- soggetto: il tecnico, che nella consegna e' il ruolo a cui questo compito e' assegnato.
...
- enunciato: il tecnico dichiara risolto un guasto.
- soggetto: il tecnico, per la ragione scritta sopra al ramo: chiudere l'allarme e' il seguito
...
- enunciato: l'amministratore annulla una corsa; il convoglio passa in SOPPRESSO e non viaggia piu'.
- soggetto: l'amministratore, che nella consegna e' il ruolo a cui questo comando e' assegnato.
- precondizione: il convoglio e' fermo in una stazione.
- rispetto alla consegna: la consegna lo scrive fra parentesi, come comando facoltativo; e' stato
...
- rispetto alla consegna: e' un'aggiunta. La consegna chiede alla centrale di *memorizzare* lo
...
*Stato di implementazione*: parziale — l'accesso e' completo; l'autorizzazione per ruolo vale per le
...
- stato di implementazione: parziale, e il pezzo che manca e' la ripartizione dei comandi operativi.
  ...
  sola lettura, ed e' un limite dichiarato nel capitolo /Progettazione/.
...
- comportamento atteso ::
  ...
  5. il tecnico manda la squadra: B passa a MANUTENZIONE;
  6. il tecnico dichiara risolto il guasto: B torna ONLINE, l'allarme risulta chiuso e conservato
     nella storia;
