...
=RestApiGateway= con 61, =IngestionService= con 23, =TrafficLogicEngine= con 3, =FaultMonitor= con
...
(=ItinerarioTratta.find("itinerario.id", Sort.by("ordine"))=) comparivano in cinque punti, tre nel
gateway e due nell'ingestione; il metodo che da quelle tratte ricava la sequenza delle stazioni
esisteva in due copie identiche con due nomi diversi (=stazioniOrdinate= nel gateway,
=stazioniOrdinateDiItinerario= nell'ingestione); i guasti ancora aperti di una sorgente erano
quattro stringhe JPQL quasi uguali, con il tipo della sorgente incollato dentro la stringa ('TRENO'
in un punto, 'STAZIONE' negli altri tre). Adesso sono =tratteOrdinateDi=, =stazioniOrdinateDi= e
