...
l'accesso ci sono per intero; dei tre comandi operativi di RF01.4 due sono completi e alla
soppressione manca la verifica della precondizione.
...
*Stato di implementazione*: parziale — l'elenco c'e' per intero, la distinzione fra guasto segnalato
...
- *perche' e' un requisito e non un dettaglio*: sono due condizioni con cause diverse e con reazioni
  ...
  senza indovinare.
- dipende da: RF02.6.
- stato di implementazione: parziale. La centrale la distinzione ce l'ha — il tipo del guasto lo
...
*Stato di implementazione*: parziale — l'invio della squadra e la chiusura di un allarme sono
completi; della soppressione manca la verifica della precondizione.
...
- stato di implementazione: completo. Il comando parte, il campo viene avvisato, i guasti aperti
  sulla stazione vengono chiusi e la stazione passa a MANUTENZIONE — e ci *resta*, che e' la parola
  del requisito. Il ritorno a ONLINE e' diventato un comando a se' (/intervento concluso/), perche'
  la fine del lavoro e' un fatto che accade dopo e che solo chi lo segue conosce: prima stava dentro
  la stessa chiamata dell'invio e lo stato non durava abbastanza per risultare a nessuno.
- *come e' stato realizzato*: il passaggio di stato non viene piu' scritto a mano sulla fotografia
  in memoria ma trattato come una conseguenza, con la stessa strada dei cambiamenti causati da un
  evento (=eventi_domino.md=), dove pero' il fatto di partenza non e' un sensore ma la decisione
  dell'operatore. Il guadagno non e' formale: da qui in poi il passaggio a MANUTENZIONE finisce in
  =Storico_Stato_Stazioni= con dentro *chi* lo ha deciso, mentre prima non ci finiva affatto, e la
  riga di =Storico_Interventi_Manutenzione= misura la durata vera del lavoro invece di due
  timestamp separati da qualche millisecondo.
- *conseguenza sull'asimmetria di RF02.3.4*: MANUTENZIONE non e' nel vocabolario del nodo, che di
  se' dichiara solo ONLINE o GUASTA. Finche' la squadra e' sul posto, quindi, il battito della
  stazione non sovrascrive lo stato deciso dalla centrale: altrimenti il primo heartbeat utile
  avrebbe cancellato la manutenzione, e saremmo tornati al punto di prima per un'altra strada.
...
- dipende da: RF02.1.3.
- stato di implementazione: completo. La chiusura c'e' e funziona, e cio' che era trattenuto riprende
  davvero la corsa; la presa in carico adesso e' un comando a se' (=POST /api/allarmi/{id}/assegna=)
  che scrive l'assegnatario sul guasto e apre la riga in =Storico_Assegnazioni_Guasti=, chiusa poi
...
sola (RF02.6). Sono adesso realizzate anche le conseguenze del guasto del convoglio
sull'infrastruttura (RF02.1.2.2), nei due casi della stazione e della tratta. Restano fuori la
ripresa dal punto giusto dopo una modifica dell'itinerario (RF02.5.2) e la strada per cui viene
trattenuto chi si trova gia' in sosta nella stazione che si guasta (RF02.1.1.2.3). La memoria
...
*Stato di implementazione*: parziale — il guasto della stazione, il guasto del convoglio con le sue
conseguenze sull'infrastruttura, la chiusura automatica e il ciclo di vita completo di presa in
carico ci sono. Resta fuori il solo RF02.1.1.2.3, per il motivo scritto li'.
...
*Stato di implementazione*: completo — il guasto viene registrato e le conseguenze
sull'infrastruttura, che prima non c'erano, adesso valgono in tutti e due i casi previsti.
...
*Stato di implementazione*: completo, in tutti e due i casi.

*Nota di lettura*: i due casi non sono due funzioni diverse ma due istanze della stessa regola, e
per questo sono stati scritti insieme. La regola e' che un nodo dichiara un fatto suo e gli altri
nodi ne traggono le conseguenze da soli, dichiarandole a loro volta: il documento
=doc/AnalisiDellaBaseDiCodice/eventi_domino.md= la descrive per esteso. Cio' che cambia fra i due
casi e' soltanto /chi/ reagisce, e a sceglierlo e' dove si trova il convoglio quando si rompe.
...
- *chi decide*: la stazione, non la centrale. E' l'unica a sapere quali convogli ha sui binari in
  questo momento, ed e' gia' in ascolto sul canale degli allarmi: vede l'avaria nell'istante in cui
  viene dichiarata. Metterlo in centrale avrebbe voluto dire che a rete caduta la conseguenza non
  vale piu', che e' esattamente cio' che RF02.1.1.2 vieta.
- stato di implementazione: completo. La stazione che riconosce fra i propri treni presenti il
  convoglio guasto si dichiara non percorribile, lo comunica come /conseguenza/ (quindi senza
  aprire un secondo guasto per la stessa avaria) e ripubblica il fatto come guasto proprio, perche'
  i convogli in avvicinamento reagiscono agli stati delle stazioni. Da li' in poi vale RF02.1.1.2
  senza nessuna aggiunta: chi e' diretto a quella stazione si ferma. Il ripristino srotola la
  catena al contrario, e la stazione torna percorribile solo se non ha altri guasti aperti suoi.
...
- dipende da: RF02.2.2.1.
- *chi decide*: il convoglio stesso. La tratta e' l'unico soggetto di questa specifica che non e' un
  processo: non ha un nodo, non ascolta e non puo' accorgersi di niente, quindi qualcuno deve
  parlare per lei. Il convoglio e' l'unico che sa dove si trovava nel momento in cui si e' rotto: la
  centrale, al piu', lo dedurrebbe dall'ultima posizione ricevuta.
- stato di implementazione: completo. La tratta ha adesso una percorribilita', che prima non
  esisteva affatto: e' la ragione per cui il caso non era realizzato, non mancava la logica ma il
  posto dove scrivere il risultato. Il convoglio guasto fuori stazione dichiara non percorribile
  l'arco che occupa e lo ripubblica come guasto con la sorgente TRATTA, cosi' gli altri convogli
  non lo imboccano; la centrale registra il cambiamento in =Storico_Stato_Tratte= con dentro quale
  convoglio l'ha causato. Vale anche per l'allarme che la centrale deduce da sola (RF02.6.2), che
  arriva al convoglio sullo stesso canale e produce la stessa dichiarazione.
- *limite*: la percorribilita' vive in memoria come tutto il resto dello stato corrente, quindi al
  riavvio della centrale ogni arco riparte percorribile finche' qualcuno non dichiara il contrario.
  E' lo stesso limite gia' dichiarato per lo stato delle stazioni.
...
- stato di implementazione: completo. L'unicita' del guasto aperto per sorgente c'e', da qualunque
  ...
  carico, preso in carico quando =OperatoreCheSeNeStaOccupandoFK= porta un assegnatario e c'e' una
  riga aperta in =Storico_Assegnazioni_Guasti=, chiuso quando =risolto= diventa vero. Il terzo stato
...
- stato di implementazione: completo, e adesso anche nella conseguenza: l'allarme dedotto viaggia
  sul canale come tutti gli altri, il convoglio che ne e' oggetto lo legge e dichiara non
  percorribile l'arco su cui e' fermo (RF02.1.2.2.2). Prima l'allarme restava una notifica e basta.
...
- *regola di registrazione*: la storia registra i *cambiamenti*, non i campionamenti. Un convoglio
  ...
  serve.
- stato di implementazione: completo. Tutte e cinque le cose elencate nell'enunciato vengono
  scritte, e vengono scritte ai cambiamenti e non a ogni campionamento.
  - *transiti, cambi di stato dei convogli e delle stazioni, guasti*: li scrive l'ingestione nel
    momento in cui l'evento arriva dal campo (=Storico_Transiti=, =Storico_Stato_Treni=,
    =Storico_Stato_Stazioni=, =Storico_Guasti=).
  - *itinerari percorsi*: una riga di =Storico_Itinerari= per ogni assegnazione di un itinerario a
    ...
    chiaro e, tratta per tratta, in =Storico_Itinerari_Tratte=: l'itinerario nel modello e' solo un
  ...
  - *assegnazioni degli operatori*: sono due cose distinte e hanno due tabelle.
    =Storico_Assegnazioni_Guasti= registra la presa in carico di un allarme
    (=POST /api/allarmi/{id}/assegna=), e si chiude alla risoluzione; chi risolve un allarme senza
    averlo prima preso in carico ne lascia comunque una, aperta e chiusa nello stesso istante.
    =Storico_Interventi_Manutenzione= registra l'invio della squadra a una stazione (RF01.4.1), che
    prima passava solo per MQTT e per la websocket e non lasciava traccia da nessuna parte.
- *nota sulla regola di registrazione*: il controllo non e' formale. Riassegnare a un convoglio
...
- *nota su cio' che ts_completamento significa*: vuol dire "qui il convoglio ha smesso di percorrere
  ...
  dell'assegnazione.
- *l'allineamento dell'avvio*: registrare i cambiamenti ha una conseguenza che va gestita, e non e'
  ...
  =AllineamentoStorico= apre la riga dei convogli che hanno un itinerario e non ne hanno una:
  e' idempotente, al secondo avvio non trova piu' niente da fare. Di quelle righe l'unica cosa che
  non si puo' sapere e' *quando* l'assegnazione e' cominciata davvero, perche' nessuno l'aveva
  scritta: il loro =ts_assegnazione= e' il momento dell'allineamento. Da li' in avanti le due cose
  coincidono.
...
- comportamento atteso ::
  ...
  5. l'operatore manda la squadra: B passa a MANUTENZIONE e ci /resta/ finche' l'intervento non
     viene dichiarato concluso, anche se nel frattempo B continua a battere;
...
- requisiti coperti :: RF02.2.3, RF02.2.4, RF02.7.
...
' i cinque servizi messi su due file: in una sola riga il grafo viene
...
note top of RF01
  ...
  tutti e cinque i servizi, quindi si fattorizza una volta
  sola invece di ripeterla cinque volte.
end note
...
note top of RF0207
  Chi dipende da quest'area, dagli altri grafi: su RF02.6
  si appoggia RF01.2, per distinguere un guasto segnalato
  da uno dedotto. La memoria storica invece non la legge
  nessun servizio: si scrive e basta.
end note
...
| RF01.4.1     | M10, M12    | S9, M11      | invio e fine intervento sono due comandi distinti: MANUTENZIONE dura finche' la squadra lavora, e il battito del nodo non la sovrascrive |
...
| RF02.1.2.2.1 | S8, S10     | T5, T9, M11  | decide la stazione, che sa quali convogli ha sui binari: si dichiara impercorribile e ripubblica il fatto, poi vale RF02.1.1.2 |
| RF02.1.2.2.2 | T4, T5      | M8, M11      | decide il convoglio, unico a sapere dove si e' rotto: la tratta non e' un processo e non puo' accorgersene da se' |
...
- RF02.1.2.2.1 e RF02.1.2.2.2 adesso ci sono, e le macchine a stati vanno ridisegnate di
  conseguenza: al convoglio serve la fase BLOCCATO_GUASTO_TRATTA accanto a quella per la stazione
  guasta (T5), alla stazione il passaggio a GUASTA causato da un convoglio sui suoi binari (S8), e
  alle tratte una macchina che prima non aveva senso perché non avevano nessuno stato. Il
  comportamento è implementato e verificato dai test, i disegni no: è un debito di
  documentazione, non di codice, e va saldato prima della consegna.
...
- RF01.4.1: il ritorno a ONLINE e' stato staccato dal comando di invio e legato alla fine
  effettiva dell'intervento, che e' un comando a se'. Da segnare come debito di documentazione:
  M12 guadagna la transizione di fine intervento e S9 resta invariata (il nodo non ha lo stato
  MANUTENZIONE, e' l'asimmetria di RF02.3.4), ma i disegni sono ancora quelli di prima.
...
- Nessuna delle nove tabelle di storico ha una schermata nella web app, ed è una scelta, non una
  dimenticanza: RF02.7 chiede di *conservare* traccia degli eventi, non di mostrarla. Transiti,
  cambi di stato, guasti, assegnazioni degli operatori e itinerari percorsi si scrivono e si
  conservano; per rileggerli serve una query sul database. Una schermata che li elenchi sarebbe
  una funzione in più, che nessun requisito chiede.
...
*** Tutte le query stanno in una classe sola

Le chiamate a Panache prima stavano dove servivano: novanta punti sparsi in otto classi, cioè
=RestApiGateway= con 56, =IngestionService= con 18, =TrafficLogicEngine= con 9, =FaultMonitor= con
3, poi =AuthController= e i due validatori dell'id con una a testa, e una perfino dentro un'entità,
=StoricoGuasto=. Adesso stanno tutte dentro =RailwayRepository= (package =persistence=), un bean
=@ApplicationScoped= con 46 metodi: nel resto della Centrale non c'è più nessun =findById=,
=persist= o =delete=.

*Il motivo non è l'ordine formale ma le copie.* Con le query scritte sul posto la stessa lettura
finiva riscritta ogni volta che serviva, e le copie erano tre. Le tratte di un itinerario in ordine
di percorrenza (=ItinerarioTratta.find("itinerario.id", Sort.by("ordine"))=) comparivano in cinque
punti, tre nel gateway e due nell'ingestione; il metodo che da quelle tratte ricava la sequenza
delle stazioni esisteva in due copie identiche con due nomi diversi (=stazioniOrdinate= nel gateway,
=stazioniOrdinateDiItinerario= nell'ingestione); i guasti ancora aperti di una sorgente erano tre
stringhe JPQL quasi uguali, con il tipo della sorgente incollato dentro la stringa ('TRENO' in un
punto, 'STAZIONE' negli altri due). Adesso sono =tratteOrdinateDi=, =stazioniOrdinateDi= e
=guastiApertiDi(sorgenteId, sorgenteTipo)=, e il tipo è un parametro invece di una costante scritta
nel testo della query.

*Le transazioni sono rimaste ai chiamanti, ed è una scelta.* Nel repository non c'è nessun
=@Transactional= e nessun =QuarkusTransaction=: i suoi metodi girano dentro la transazione di chi
li chiama. La ragione è che i confini attuali non sono casuali. Il gateway apre la transazione
sull'endpoint, mentre l'ingestione e il =FaultMonitor= chiamano =QuarkusTransaction.requiringNew()=
*dentro* il =try=, così il commit avviene prima delle notifiche su MQTT e WebSocket e una
violazione di vincolo non risale fino al connettore reattivo spegnendo il canale (è il motivo per
cui i consumatori non sono =@Transactional=, spiegato in C4). Se il repository si mettesse a
dichiarare transazioni per conto suo, quei confini si sposterebbero senza che si veda dal codice
del chiamante.

*Quello che il repository non fa.* Non costruisce =Response=, non valida niente e non scrive
messaggi d'errore: codici HTTP e testi restano nel gateway, accanto alla regola che li produce. E
non tocca lo stato in RAM: il =TrafficLogicEngine= lo usa una volta sola, all'avvio, per riempirsi.

*Il limite, detto chiaro.* Non è il pattern Repository dei libri, quello in cui il dominio non sa
come è fatta la persistenza. Le entità restano =PanacheEntityBase= in stile /active record/, e il
repository restituisce oggetti *gestiti* da Hibernate: chi riceve un =Treno= e gli cambia un campo
se lo ritrova scritto al commit, senza chiamare nessun metodo di salvataggio. È esattamente quello
che succede in =salvaStatoTreno=, che assegna =dbTreno.stato= e non salva niente. Qui dunque il
...
*Perché una classe sola e non un DAO per tabella.* Le entità toccate sono tredici, ma le query per
...
=telemetria_treni= ha una colonna che si chiama =timestamp= e in H2 2.x e' una parola riservata, e
...
entità Panache in stile /active record/, ma i loro metodi statici li chiama soltanto
=RailwayRepository= (il diagramma =C5=), che è l'unica classe della Centrale a parlare con
PostgreSQL.
...
- =Tratte=: un arco fra due stazioni (=StazionePartenzaFK= -> =StazioneArrivoFK=) più il
  tempo nominale di percorrenza in minuti, che serve poi al calcolo del ritardo. La sua
  *percorribilità* (RF02.1.2.2.2) non è una colonna qui: è stato corrente, quindi vive in
  memoria come quello delle stazioni, e a database va il cambiamento, cioè una riga di
  =Storico_Stato_Tratte=
...
- =Utenti=: anagrafica degli operatori (nome, cognome, matricola univoca, tipo). Da quando
  ...
  perchè =Guasti= ha una chiave esterna verso =id_utente=, cioè serve a dire quale operatore
  sta seguendo un guasto; il collegamento fra l'utente di Keycloak e la riga qui dentro è la
  *matricola*, che viaggia dentro il token. Gli storici invece la chiave esterna non ce
...
=Storico_Stato_Stazioni=, =Storico_Stato_Tratte=, =Storico_Itinerari=,
=Storico_Itinerari_Tratte=, =Storico_Assegnazioni_Guasti=,
=Storico_Interventi_Manutenzione=): sono tabelle
...
operatori: un itinerario percorso è una riga di =Storico_Itinerari= (aperta all'assegnazione,
chiusa quando finisce) più una riga di =Storico_Itinerari_Tratte= per ogni tratta, perché
...
un allarme (=Storico_Assegnazioni_Guasti=) e l'invio fisico di una squadra a una stazione
(=Storico_Interventi_Manutenzione=).
...
comandi operativi (=/sopprimi=, =/risolvi=, =/assegna=, =/manutenzione=) a entrambi, tutte le altre
...
- requisiti coperti: nessuno per intero, e' l'orchestratore: RF02.5.1 parzialmente (lo stato SOPPRESSO porta la fase a TERMINATO) e RF02.1.2.2 parzialmente, perche' qui l'EMERGENZA congela l'avanzamento del convoglio; il contagio all'infrastruttura non nasce dal tick ma dalla lettura dell'alert, ed e' in T5 e nel gateway
...
- requisiti coperti: RF02.1.1.2.2 parzialmente, la stazione da sola non si ripara: torna percorribile solo con il RESOLVED deciso dalla centrale in M10; RF01.4.1 parzialmente lato campo, perche' e' qui che arriva l'invio della squadra di manutenzione; RF02.1.2.2.1 completo, perche' e' qui che la stazione riconosce il convoglio guasto sui propri binari e si dichiara non percorribile
...
- requisiti coperti: RF02.3.1 completo lato centrale (convoglio, stazione, tratta e orario di entrata e di uscita, a db e in cache), RF03.3 completo perche' e' il punto in cui le due fonti dello stesso passaggio restano trattate separatamente invece di sostituirsi a vicenda, RF02.7 e RF02.2.2.1 parzialmente (onPassaggio aggiorna posizione sulla tratta e prossima stazione)
...
note bottom of Chiuso
  ...
  e ogni uscita verrebbe annunciato due volte sulla stessa
  WebSocket: la sorgente ufficiale del passaggio e' il
  sensore di terra.
end note
...
note left of Aperto
  ...
  Nello storico del transito finisce anche la tratta: la stazione
...
Aperto --> PresoInCarico : M10 - POST /allarmi/{id}/assegna /\nassegnatario sul guasto e riga aperta\nin Storico_Assegnazioni_Guasti
PresoInCarico --> PresoInCarico : M10 - un altro operatore lo prende in carico /\nchiude la riga del primo e ne apre una per il secondo
PresoInCarico --> Risolto : M10 - l'operatore risolve l'allarme /\nRESOLVED su railway/alerts,\nchiude l'assegnazione
Aperto --> Risolto : M10 - l'operatore risolve l'allarme\noppure invia la manutenzione /\nRESOLVED su railway/alerts, presa in carico\nimplicita: la riga si apre e si chiude insieme
...
note left of PresoInCarico
  I tre stati che chiede RF02.1.3 ci sono tutti, ma "preso in carico"
  non e' un valore di una colonna: e' la presenza dell'assegnatario.
  Un guasto sta qui quando OperatoreCheSeNeStaOccupandoFK punta a
  qualcuno e Storico_Assegnazioni_Guasti ha una riga aperta per lui;
  passa a Risolto quando "risolto" diventa vero e quella riga si
  chiude. L'operatore lo dice il token, non il corpo della richiesta,
  quindi non se lo puo' scegliere chi chiama.

  La chiusura automatica (M3, M4) puo' scattare anche su un guasto
  gia' preso in carico: la condizione e' rientrata da sola mentre
  qualcuno ci stava lavorando. La freccia non e' disegnata per non
  incrociare mezzo diagramma, ma il caso e' gestito, e chiude anche
  la riga dell'assegnazione: senza, quell'operatore risulterebbe al
  lavoro per sempre su un allarme gia' finito.
end note
...
- requisiti coperti: RF02.6.1, RF02.6.2 e RF02.6.3 completi: la stazione silenziosa oltre T_sil, il convoglio fermo fuori stazione oltre T_fer e la fotografia dello stato ogni P_ria, cioe' tutto cio' che i nodi di campo non sono in grado di segnalare da soli; RF01.1.5 e RF01.2.2 parzialmente. L'allarme dedotto sul convoglio fermo contribuisce anche a RF02.1.2.2.2, ma non e' questa macchina a rendere non percorribile la tratta: e' il convoglio che lo legge e lo dichiara
...
- requisiti coperti: RF01.3 completo insieme a M9, RF03.5 completo lato centrale perche' e' da qui che l'itinerario viene servito ai convogli, RF02.5.1 completo e RF02.5.2 parzialmente (da qui partono STOP e ITINERARIO_AGGIORNATO, ma il punto da cui si riprende lo decide il treno in T3), RF01.3.5 parzialmente perche' l'integrita' alle eliminazioni e' affidata ai vincoli del db. Dei tre comandi operativi RF01.4.1 e RF01.4.2 sono completi (l'invio della squadra e la fine dell'intervento sono due comandi distinti, vedi la nota sotto e M12; presa in carico e chiusura passano tutte e due da qui e lasciano il nome dell'operatore, M7), mentre RF01.4.3 resta parziale perche' la precondizione "convoglio fermo in stazione" non viene verificata
...
note right of MANUTENZIONE
  ...
  campo e chiude i guasti aperti, dentro dispacciaManutenzione().
  E' anche uno stato protetto, perche' il watchdog salta le stazioni
  in manutenzione e non le porta a OFFLINE.
  Da tenere presente quando si legge RF01.4.1: adesso lo stato DURA,
  perche' il ritorno a ONLINE e' un comando a se' (fine intervento) e
  non piu' la seconda meta' della stessa chiamata. Viene annunciato
  sulla websocket, quindi il frontend, che calcola operatorsDispatched
  come stato == MANUTENZIONE, accende davvero la spia. Il battito
  della stazione non lo cancella: il nodo dichiara di se' solo ONLINE
  o GUASTA (RF02.3.4) e questo stato non e' suo. Il vecchio testo
  diceva che il ritorno a ONLINE andrebbe staccato dal comando e legato
...
- la corsia =PostgreSQL= e' il database, non una classe: le frecce che ci arrivano passano tutte dal
  =RailwayRepository=, e per questo l'etichetta porta il nome del suo metodo. Non l'ho disegnato come
  partecipante a se' perche' avrebbe aggiunto una corsia a quindici figure senza dire niente di
  nuovo; chi lo chiama e per che cosa si vede in =C5=
...
'    it.uni.reti2.persistence.RailwayRepository#esisteTreno
'
...
'    it.uni.reti2.persistence.RailwayRepository#esisteStazione
'
...
loop finche' l'esito e' RIPROVA (sleep 5s)
    ...
    opt sono passati almeno 15s dall'ultimo tentativo
        ...
        deactivate BROKER
        activate EXIST
        EXIST -> PG : repository.esisteStazione(stazioneId)
        activate PG
        PG --> EXIST : entita' oppure null
        deactivate PG
...
loop finche' l'itinerario e' vuoto, un tentativo ogni 15 secondi
    ...
    deactivate FILTRO
    activate REST
    REST -> PG : repository.trovaTreno(id)\nrepository.tratteOrdinateDi(itinerarioId)
    activate PG
...
alt stazione sconosciuta
    ...
    opt statoCambiato
        ING -> PG : QuarkusTransaction.requiringNew()\nrepository.salvaStoricoStatoStazione(...)
        note right of PG
...
- requisiti coperti: RF03.3 completo, RF02.7 parzialmente perche' e' qui che nasce lo storico dei transiti, RF02.2.4 per i passaggi spaiati dei capolinea
...
ING -> PG : repository.salvaGuasto + salvaStoricoGuasto
...
- requisiti coperti: RF02.1.2.1 completo, RF02.1.3 parzialmente per l'unicita' per sorgente, RF01.2.1 per l'elenco degli allarmi aperti; RF02.1.2.2 comincia qui ma si compie sul campo, ed e' spiegato nella nota
...
alt esiste gia' un guasto aperto per questa sorgente e tipo
    ...
    ING -> PG : QuarkusTransaction.requiringNew()\nrepository.salvaGuasto + salvaStoricoGuasto
...
note over ING, PG
  ...
  allarmi aperti. L'alert torna anche al convoglio che l'ha mandato,
  ed e' li' che nasce la conseguenza (RF02.1.2.2): se era fermo in una
  stazione reagisce la stazione, che si dichiara non percorribile; se
  era in linea reagisce lui, dichiarando non percorribile l'arco che
  occupa. In tutti e due i casi la catena e' la stessa, quindi
  l'operatore vede una sola avaria e non due.
end note
...
loop ogni 10 secondi
    ...
    alt stato gia' OFFLINE o MANUTENZIONE
        ...
        opt nessun guasto "sensore_offline" gia' aperto per questa stazione
            ...
            FM -> PG : repository.salvaGuasto + salvaStoricoGuasto
            note right of PG
...
- requisiti coperti: RF02.6.2 completo, e con lui RF02.1.2.2.2: l'allarme dedotto torna al convoglio, che dichiara non percorribile l'arco su cui e' fermo
...
note over BROKER
  L'alert finisce anche sul campo. Gli altri convogli non reagiscono
  a un guasto di TRENO, ma il convoglio che ne e' oggetto si': legge
  che l'anomalia riguarda lui, si accorge di essere in linea e
  dichiara non percorribile l'arco che occupa (RF02.1.2.2.2). Da li'
  in poi e' quel fatto, non l'allarme, a fermare chi doveva
  imboccarlo. La severita' qui non conta: warning o critico, il
  convoglio e' fermo di traverso lo stesso.
end note
...
alt credenziali sbagliate
    ...
    activate CTRL
    CTRL -> PG : repository.trovaUtentePerMatricola(claim "matricola")
    activate PG
...
alt token assente, contraffatto, scaduto o audience sbagliata
    ...
    else GET, oppure /sopprimi, /risolvi, /assegna, /manutenzione
...
- requisiti coperti: RF01.4.1 completo (il comando parte, il campo viene avvisato e lo stato MANUTENZIONE dura fino al comando di fine intervento, vedi M12), RF01.4.2 parzialmente (la chiusura c'e', la presa in carico no), RF02.1.3 parzialmente per la sola chiusura del ciclo di vita del guasto
...
alt stazione non in cache
    ...
    REST -> PG : repository.guastiApertiDi(id, "STAZIONE")
    activate PG
    ...
    note right of CACHE
      Qui viene scritto MANUTENZIONE, e basta: la stazione ci resta.
      ONLINE torna con il comando di fine intervento, che e' una
      chiamata diversa fatta quando il lavoro e' finito davvero.
      RF01.4.1 chiede che "la stazione risulti in MANUTENZIONE", e
      adesso lo e' per tutta la durata dell'intervento. Il passaggio
      lascia anche la sua riga di storico, con dentro chi lo ha
      deciso, perche' non e' piu' una scrittura diretta in cache ma
      una conseguenza applicata dalla porta unica. Vedi la nota di M12.
    end note
end
deactivate REST
...
== Presa in carico di un allarme ==

USER -> API : bottone "Prendi in carico"
API -> FILTRO : POST /api/allarmi/{id}/assegna\nBearer <token>
FILTRO -> REST : assegnaAllarme(id)
activate REST
alt guasto gia' risolto
    REST --> API : 409 {"errore":"Il guasto e' gia' stato risolto"}
else guasto aperto
    REST -> REST : operatoreCollegato.dati()\nmatricola e ruolo dal token
    REST -> PG : Guasto.operatore = utente di quella matricola
    REST -> PG : Storico_Assegnazioni_Guasti:\nriga aperta (ts_risoluzione null)
    REST -> API : 200 guasto aggiornato
    note right of REST
      Se dell'allarme si stava gia' occupando un altro, la sua riga
      viene chiusa e se ne apre una nuova: il passaggio di mano resta
      scritto e a lavorarci non risulta mai piu' di uno per volta.
      Riprendere in carico un allarme che si ha gia' non scrive niente.
    end note
end
deactivate REST
...
alt guasto inesistente
    ...
    REST -> REST : chiudiGuasto(guasto, operatoreCollegato.dati())
...
note over USER, PG
  Il Tecnico non e' piu' un attore anonimo. Il token non serve solo al
  filtro per far passare il comando: OperatoreCorrente ne ricava
  matricola e ruolo, e risolviAllarme e dispacciaManutenzione li
  scrivono su Guasto.operatore, su Storico_Assegnazioni_Guasti e su
  Storico_Interventi_Manutenzione. La presa in carico ha un comando
  suo (POST /api/allarmi/{id}/assegna) che apre la riga; questo
  diagramma mostra la chiusura, che e' quella che la chiude.

  L'identita' viene dal token e non dal corpo della richiesta, quindi
  il chiamante non puo' firmare a nome di un altro. Se l'anagrafica
  Utenti non ha la riga corrispondente si scrive comunque la
  matricola, che e' cio' che identifica la persona in Keycloak.
end note
...
alt itinerario inesistente
    ...
    opt sono arrivate almeno due stazioni
        REST -> PG : repository.eliminaTratteDellItinerario(id)
...
alt la stazione e' estremo di almeno una tratta
    ...
    REST -> PG : cancella i Transiti vivi, poi la Stazione\n(lo storico non si tocca: RF02.7)
...
'    src/hooks/useRealtimeUpdates.ts                      (le subscribe agli eventi)
...
deactivate WS
HOOK -> WSC : subscribe("TELEMETRY" | "HEARTBEAT" | "ALERT")
deactivate HOOK
...
| RF01.4.1     | SQ14       |              | il comando parte, il campo viene avvisato e MANUTENZIONE dura fino al comando di fine intervento |
...
| RF02.1.2.1   | SQ09       |              |                                                                               |
| RF02.1.2.2.1 | SQ09       | SQ08         | l'avaria di bordo di SQ09 rende impercorribile la stazione, e da li' vale SQ08 |
| RF02.1.2.2.2 | SQ09, SQ11 |              | il convoglio guasto fuori stazione dichiara non percorribile l'arco che occupa |
...
Sei diagrammi e una mappa: =C1= il nodo Treno, =C2= il nodo Stazione, la mappa della Centrale che
tiene insieme =C3=, =C4= e =C5=, e =C6= la Web App. Le entita' JPA non hanno un diagramma a se':
stanno nel capitolo /Modello/.
...
'
'  I tre archi fra le due viste sono verificati sul codice
...
'  dell'altro.
'
'  Sotto tutte e due c'e' pero' una terza cosa in comune, il
'  RailwayRepository (C5). Lo iniettano sette classi: cinque di C4
'  (IngestionService, FaultMonitor, i due validatori e il
'  TrafficLogicEngine, che pero' lo usa solo all'avvio) e due di C3
'  (RestApiGateway e AuthController). Non sta in nessuna delle due viste
'  perche' non e' di nessuna delle due: e' il piano di sotto.
' =========================================================================
...
skinparam rectangle<<persistenza>> {
    BackgroundColor #FEF7E0
    BorderColor #B06000
}
skinparam note {
...
rectangle "C5 - la persistenza\n<size:11>non e' una vista: sta sotto tutte e due</size>" as C5 <<persistenza>> {
    rectangle "RailwayRepository\n<size:11>46 metodi, l'unico punto della Centrale</size>\n<size:11>che parla con PostgreSQL. Nessuna</size>\n<size:11>transazione propria: gira in quella</size>\n<size:11>di chi lo chiama</size>" as REPO <<classe>>
}
...
C3 --> C5 : gli endpoint leggono\ne scrivono solo da qui
C4 --> C5 : scritture del campo, validazione\ndegli id, riempimento della cache

note as CONFINE
  Il confine fra C3 e C4 non e' una chiamata di servizio: sul piano delle
  due viste i lati si incontrano su questi due oggetti e nient'altro, e
  ognuno dei due sta in casa dell'altro. E' per questo che sono due
  diagrammi distinti e non due meta' dello stesso.

  Il RailwayRepository e' un incontro di natura diversa: non sta in casa di
  nessuno dei due, sta sotto tutti e due, ed e' per questo che ha un
  riquadro suo. Da quando c'e' lui, AuthController e i due validatori non
  interrogano piu' le entita' JPA per conto loro: erano proprio le tre
  classi che in questa mappa restavano senza nessun arco.
end note
CONFINE .. C3
...
legend bottom
  ...
  | riquadro giallo | il piano di sotto: non e' di nessuna delle due viste, ma le serve tutte e due |
...
'
'  Da quando le query stanno nel RailwayRepository (C5) qui dentro non c'e'
'  piu' nessuna chiamata a Panache: gli endpoint validano, decidono il
'  codice HTTP e rispondono, mentre a leggere e scrivere ci va il
'  repository, nella transazione aperta dall'endpoint stesso.
' =========================================================================
...
class AuthController <<@Path("/api/auth")>> {
  - operatore : OperatoreCorrente <<@Inject>>
  + profilo() : Response
  - componiNomeCompleto(nome, cognome, matricola) : String
  - iniziali(nome, cognome, matricola) : String
}

class OperatoreCorrente <<@RequestScoped>> <<sicurezza>> {
  - identita : SecurityIdentity <<@Inject>>
  - repository : RailwayRepository <<@Inject>>
  + anonimo() : boolean
  + matricola() : String
  + ruolo() : String
  + inAnagrafica() : Utente
  + dati() : DatiOperatore
  + claim(nomeClaim : String) : String
}
...
class RestApiGateway <<@Path("/api")>> {
  ...
  - chiudiGuasto(guasto : Guasto, operatore : DatiOperatore) : void
...
OperatoreCorrente --> SecurityIdentity : getPrincipal(),\nclaim "matricola", hasRole(...)
OperatoreCorrente --> "RailwayRepository\n<size:11>(C5)</size>" : trovaUtentePerMatricola:\nritrova l'id_utente partendo\ndalla matricola del token
AuthController --> OperatoreCorrente : chi ha presentato il token
RestApiGateway --> OperatoreCorrente : chi ha dato il comando,\nda scrivere negli storici\ndelle assegnazioni (RF02.7)
...
RestApiGateway --> "RailwayRepository\n<size:11>(C5)</size>" : ogni lettura e ogni scrittura\nsul database, dentro la\ntransazione dell'endpoint
...
'
'  Le quattro classi di questa vista che toccano il database non lo fanno
'  piu' da se': le query stanno nel RailwayRepository (C5). Qui restano la
'  decisione di che cosa scrivere e il confine della transazione, che per
'  l'ingestione e per il watchdog e' QuarkusTransaction.requiringNew()
'  chiamato dentro il try.
' =========================================================================
...
IngestionService --> "RailwayRepository\n<size:11>(C5)</size>" : tutte le scritture del campo,\nstorico compreso, dentro la\ntransazione aperta qui
...
FaultMonitor --> "RailwayRepository\n<size:11>(C5)</size>" : il guasto automatico, la sua riga\ndi storico e l'evento di audit

ExistIdForEdge --> "RailwayRepository\n<size:11>(C5)</size>" : esisteTreno
ExistIdForEdgeStazione --> "RailwayRepository\n<size:11>(C5)</size>" : esisteStazione

TrafficLogicEngine --> "RailwayRepository\n<size:11>(C5)</size>" : tutteLeStazioni, tuttiITreni,\nguastiNonRisolti: solo all'avvio

note right of TrafficLogicEngine
  E' una cache in RAM, non un secondo database: viene riempita
  all'avvio dalle tre letture del repository e poi tenuta viva
  dai messaggi del campo.
...
*** Il componente Centrale: la persistenza (C5)
- non e' una terza vista ma il piano sotto le altre due: e' l'unica classe della Centrale che chiama Panache, e la usano sia il lato operatore (C3) sia il lato campo (C4)
- requisiti coperti: nessuno in proprio; da qui passano le scritture della memoria storica (RF02.7) e le letture dell'anagrafica che servono a RF01.3
#+begin_src  plantuml file: classi_05_persistenza.png
@startuml C5 - Classi della persistenza della Centrale
' =========================================================================
'  DIAGRAMMA DELLE CLASSI C5 - Centrale Operativa, la persistenza
'
'  Sorgente: ServeCentraleOperativa/src/main/java/it/uni/reti2/
'    persistence/RailwayRepository.java
'
'  E' l'unico punto della Centrale che chiama Panache. Le stesse chiamate
'  prima erano novanta, sparse in otto classi: RestApiGateway 56,
'  IngestionService 18, TrafficLogicEngine 9, FaultMonitor 3,
'  AuthController 1, i due validatori dell'id 1 a testa e una perfino
'  dentro l'entita' StoricoGuasto.
'
'  I sette riquadri intorno sono i clienti (a sinistra quelli di C3, a
'  destra quelli di C4) e la freccia dice per che cosa lo usano. Le entita'
'  in basso sono le tredici che il repository tocca, e stanno nel capitolo
'  Modello.
' =========================================================================
...
skinparam class<<persistenza>> {
    BackgroundColor #FEF7E0
    BorderColor #B06000
}
skinparam class<<campo>> {
    BackgroundColor #E6F4EA
    BorderColor #137333
}
skinparam class<<modello>> {
    BackgroundColor #F1F3F4
    BorderColor #9AA0A6
}
skinparam note {
...
title C5 - Centrale Operativa: l'unica classe che parla con PostgreSQL

class RestApiGateway <<C3>>
class AuthController <<C3>>
class IngestionService <<C4>> <<campo>>
class FaultMonitor <<C4>> <<campo>>
class ExistIdForEdge <<C4>> <<campo>>
class ExistIdForEdgeStazione <<C4>> <<campo>>
class TrafficLogicEngine <<C4>> <<campo>>

class AllineamentoStorico <<ApplicationScoped>> <<persistenza>> {
  - repository : RailwayRepository <<@Inject>>
  ~ onStart(ev : StartupEvent) : void
}

class RailwayRepository <<ApplicationScoped>> <<persistenza>> {
  .. stazioni ..
  + trovaStazione(id : String) : Stazione
  + esisteStazione(id : String) : boolean
  + tutteLeStazioni() : List<Stazione>
  + salvaStazione(stazione : Stazione) : void
  + eliminaStazione(stazione : Stazione) : void
  + contaTratteConStazione(idStazione : String) : long
  + eliminaTransitiDiStazione(idStazione : String) : long
  .. treni ..
  + trovaTreno(id : String) : Treno
  + esisteTreno(id : String) : boolean
  + tuttiITreni() : List<Treno>
  + treniDellItinerario(idItinerario : String) : List<Treno>
  + contaTreniInPosizioneSuTratta(idTratta : String) : long
  + salvaTreno(treno : Treno) : void
  + eliminaTreno(treno : Treno) : void
  + eliminaTransitiDiTreno(idTreno : String) : long
  .. itinerari e tratte ..
  + trovaItinerario(id : String) : Itinerario
  + tuttiGliItinerari() : List<Itinerario>
  + salvaItinerario(itinerario : Itinerario) : void
  + eliminaItinerario(itinerario : Itinerario) : void
  + trovaTratta(id : String) : Tratta
  + tutteLeTratte() : List<Tratta>
  + trovaTrattaFra(idPartenza, idArrivo : String) : Tratta
  + salvaTratta(tratta : Tratta) : void
  + eliminaTratta(tratta : Tratta) : void
  + tratteOrdinateDi(idItinerario : String) : List<ItinerarioTratta>
  + stazioniOrdinateDi(idItinerario : String) : List<String>
  + contaItinerariCheUsano(idTratta : String) : long
  + contaAltriItinerariCheUsano(idTratta, idItinerario) : long
  + salvaItinerarioTratta(riga : ItinerarioTratta) : void
  + eliminaTratteDellItinerario(idItinerario : String) : long
  .. transiti ..
  + trovaTransitoAperto(idTreno, idStazione : String) : Transito
  + salvaTransito(transito : Transito) : void
  + contaTransitiSuTratta(idTratta : String) : long
  + ultimiTransitiStorici(quanti : int) : List<StoricoTransito>
  .. guasti ..
  + trovaGuasto(id : String) : Guasto
  + tuttiIGuastiRecenti() : List<Guasto>
  + guastiApertiDi(sorgenteId, sorgenteTipo : String) : List<Guasto>
  + guastiNonRisolti() : List<Guasto>
  + salvaGuasto(guasto : Guasto) : void
  .. storici ..
  + trovaStoricoGuasto(guastoId : String) : StoricoGuasto
  + salvaStoricoGuasto(guasto : Guasto) : void
  + salvaStoricoStatoTreno(treno : Treno, statoPrecedente : String) : void
  + salvaStoricoStatoStazione(stazione, stato, statoPrecedente) : void
  + salvaStoricoTransito(transito : Transito) : void
  - nomeDellaSorgente(guasto : Guasto) : String
  .. itinerari percorsi (RF02.7) ..
  + itinerarioPercorsoAperto(idTreno : String) : StoricoItinerario
  + registraAssegnazioneItinerario(idTreno : String, itinerario : Itinerario) : void
  + registraFineItinerario(idTreno : String) : void
  + allineaItinerariPercorsi() : int
  .. assegnazioni degli operatori (RF02.7) ..
  + assegnazioneApertaDi(guastoId : String) : StoricoAssegnazioneGuasto
  + apriAssegnazioneGuasto(guasto : Guasto, operatore : DatiOperatore) : void
  + chiudiAssegnazioneGuasto(guasto : Guasto, operatore : DatiOperatore) : void
  + salvaStoricoInterventoManutenzione(stazione, guastoId, operatore, statoPrima, statoDopo, tsInvio) : void
  .. utenti ..
  + trovaUtentePerMatricola(matricola : String) : Utente
}

class "entita' JPA\n<size:11>Stazione, Treno, Tratta, Itinerario, ItinerarioTratta,</size>\n<size:11>Transito, Guasto, Utente e i nove storici</size>" as ENTITA <<modello>>

RestApiGateway --> RailwayRepository : anagrafica, comandi operativi,\nitinerario dei nodi di campo
AuthController --> RailwayRepository : trovaUtentePerMatricola\n(passando da OperatoreCorrente)
IngestionService --> RailwayRepository : stato dei treni, transiti,\nguasti e storici
FaultMonitor --> RailwayRepository : guasto automatico,\nstorico ed evento di audit
ExistIdForEdge --> RailwayRepository : esisteTreno
ExistIdForEdgeStazione --> RailwayRepository : esisteStazione
TrafficLogicEngine --> RailwayRepository : le tre letture dell'avvio
AllineamentoStorico --> RailwayRepository : all'avvio apre la riga dei convogli\ngia' assegnati, che nessun cambiamento\nregistrerebbe mai (RF02.7)

RailwayRepository ..> ENTITA : findById, list, persist, delete\n(metodi statici di Panache)

note right of RailwayRepository
  Tre cose che questa classe NON fa, e sono scelte:

  1. non apre transazioni. Niente @Transactional e niente
     QuarkusTransaction: i confini restano nei chiamanti, dove
     servono cosi' come sono (l'ingestione e il FaultMonitor
     devono chiudere la transazione DENTRO il try, se no le
     notifiche partirebbero prima del commit).
  2. non decide niente: nessuna Response, nessuna validazione,
     nessun messaggio d'errore. Quelli restano in C3.
  3. non tocca la cache in RAM: il TrafficLogicEngine e' un'altra
     cosa, e infatti lo usa solo all'avvio.
end note

note bottom of ENTITA
  Le entita' restano PanacheEntityBase in stile active record, quindi
  questo non e' il pattern Repository dei libri: gli oggetti tornano
  GESTITI da Hibernate e chi ne cambia un campo se lo ritrova scritto
  al commit, senza chiamare nessun metodo di salvataggio. Qui si
  raccolgono le query, non si isola il modello dal database.
end note
...
P4 --> railwayStore
P5 --> railwayStore
P5 --> authStore : il nome di chi\nprende in carico un allarme
P6 --> railwayStore
...
useRealtimeUpdates --> wsClient : connect + le subscribe agli eventi
...
| RF02.7              |            | modello, C5     | le nove tabelle di storico stanno nel Modello e non in un diagramma; a scriverle e' tutte il repository, che e' l'unica classe che parla col database |
...
(=TrainDB=, =DBLocale=, =TrafficLogicEngine=), che tengono in RAM le variabili ma non le muovono, i
client (=Sensori=, =SecureHttpClient=, le pagine della web app) e il =RailwayRepository=, che le
